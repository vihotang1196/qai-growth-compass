...
1. **[判断标准](#判断标准--这个项目反复用到的十四条)** —— 十四条。它们不是格言,是每一条都对应一次真实的返工。
   不读这节,后面很多设计会显得像洁癖。
2. **[八道门](#八道门--每一道都是撞出来的)** —— 构建链上的七道守卫各自守什么、**因为踩了什么坑才加的**。
...
当前分支基线:`main` = `596e2d2`。测试基线:**Node 287 / Deno 132,八道门全绿**。
...
**为什么现在不改**:改 resync 要重新部署 Edge Functions,而那会让 Disable 验收
清单里第 7 条的结论失效(它验的正是那个函数)。**等验收跑完再改。**
...
### 6. ⚠️ `/api/cron/retention` 从公网回 200 + HTML —— **但那条 cron 一直在正常跑**
...
原本据此推断「函数没执行 → 这条 cron 从 Stage 4 起从没跑过」。**那是错的。**

`assessment-maintenance` 的 Invocations:**连续五天每天准点一次**
(排期 `17 3 * * *` UTC,记录显示 11:17 UTC+8 —— 时区正好对上,这本身是第二重确认)。
**留存清理一直在做。**

所以真正的事实是:**Vercel Cron 走的那条路与公网 curl 不是同一条**。
公网那条为什么被静态产物接住,**仍然没有解释** —— 但它不再紧急,
因为它不影响 cron 本身;它影响的是「人能不能手动 curl 触发」。

**这一段没有删掉,是因为走错的过程本身是标本**:那条推理链有五步(正则、文件差异、
一节分析、一道门、门里的断言),而第一步是错的,后面四步全白做 ——
而且每一步都产出了「看起来是进展」的东西。已立成[判断标准 14](#14-有直接观测可用时先观测再推理)。

## 还没解释的那件

同目录两个函数表现不同:`pdf-sweep` 回 401 JSON,`retention` 回 200 HTML。
**这不是设计出来的状态**,值得查,但不紧急。

**下一步只做观测,不做推理**:看 Vercel Logs 里筛 `api/cron/retention` 这个函数 ——
① 每天 03:17 UTC 那几次在不在、状态是什么;② **手动 curl 那次有没有产生日志行**。
②如果没有,就说明请求压根没到达函数,差异在边缘路由而不是函数本身。一次点击定性。

线索(来自 Vercel 的 cron 排查页,不是推断):cron **不跟随重定向**,
而且那页专门提到过缓存问题。两条都还没被验证到这个案例上。
...
⚠️ **它证明的是「公网可达」,不是「cron 在跑」。** 第一版把判据写成「函数有没有执行」,
并在失败信息里断言「所以这条日程会每天成功而从不运行」—— 那个断言被 Invocations 推翻了。
**现在它的失败信息里写着「这不代表 cron 没在跑,retention 就是反例」**,
并指向真正算证据的东西(下游调用记录 / 函数自己的日志)。
基于推断加的守卫,要把「这是推断」写进它自己的失败信息里(判断标准 14 推论三)。

它仍然有用:它捕捉的是**同目录两个 cron 表现不同**这个差异,而那不是设计出来的状态;
另外「人手动 curl 触发」这条路全靠它。理由:
...
推论三:**基于推断加的守卫,要把「这是推断」写进它自己的失败信息里。**
那道 smoke 检查的第一版把推断写成了断言,于是它会把同一个错误论断复述给下一个人。
现在它的措辞是「这不代表 cron 没在跑,retention 就是反例」。
...
- **`check:deno`** 拦下了 `assessment-admin` 里那份**自己的** `RosterRow` 类型 ——
  ...
  这正是「[verify 靠人记得跑](#八道门--每一道都是撞出来的)」那个已知洞会漏掉的东西。
...
- Stage 10 的四个模块届时也要各自决定测试行的处理 —— 漏斗与看板大概率该跟名单页一致
  (默认隐藏 + 开关),而不是各写一套
...
- 2026-08-07 — **交接整理**:补上四类只活在对话里的东西 —— ①「[判断标准](#判断标准--这个项目反复用到的十四条)」七条(每条附它对应的那次返工)②「[七道门](#八道门--每一道都是撞出来的)」合并成一节,每道门写明**因为撞了什么才加**③ 状态总览按实际进度更正(4/5/6 原本还写着「未开始」,Stage 6 是 15 题不是 24 题)④ 新增「[当前未完成](#当前未完成)」,含已确认未修的 `assessment_entitlements.status` bug
- 2026-07-31 — rev1 初稿
