...
`supabase migration list` **不带**参数时(非 TTY)输出的就是 JSON;
而带上 `--output json` 反而输出表格、JSON 那一行消失。
两种都跑过并比对了首字符与末行 —— **一个「看起来更明确」的参数把输出变得更难解析**,
那种反直觉只能靠实测发现。第一版我按直觉加了 `--output json`,于是真实那次直接红
(而它红在 fail-closed 那条路上,所以至少没有假绿)。
