...
1. **[判断标准](#判断标准--这个项目反复用到的十九条)** —— 十九条。它们不是格言,是每一条都对应一次真实的返工。
   不读这节,后面很多设计会显得像洁癖。
2. **[十道门](#十道门--每一道都是撞出来的)** —— 构建链上的十道守卫各自守什么、**因为踩了什么坑才加的**。
...
当前分支基线:`main` = `fd05c59`。测试基线:**Node 334 / Deno 178,十道门全绿**。
...
- **`check:deno`** 拦下了 `assessment-admin` 里那份**自己的** `RosterRow` 类型 ——
  ...
  这正是「[verify 靠人记得跑](#十道门--每一道都是撞出来的)」那个已知洞会漏掉的东西。
...
而 PROGRESS 里[早就写着](#十道门--每一道都是撞出来的)「加人之后上 GitHub Actions 跑 `npm run verify`」。
...
- 2026-08-07 — **交接整理**:补上四类只活在对话里的东西 —— ①「[判断标准](#判断标准--这个项目反复用到的十九条)」七条(每条附它对应的那次返工)②「[七道门](#十道门--每一道都是撞出来的)」合并成一节,每道门写明**因为撞了什么才加**③ 状态总览按实际进度更正(4/5/6 原本还写着「未开始」,Stage 6 是 15 题不是 24 题)④ 新增「[当前未完成](#当前未完成)」,含已确认未修的 `assessment_entitlements.status` bug
- 2026-07-31 — rev1 初稿
