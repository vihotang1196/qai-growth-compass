...
# warnings 落库 + 三条追补 + 名单页那一格

## 这一列要修的不是显示,是让一个**从来没成立过的组合**成立

代码注释里写着「回落 + warning,应该让它可见(warning + Admin 能看到)而不是阻塞客户」——
而 `Admin 能看到` 这半**从来没有实现过**。所以在这一列之前,
过去每一次选「warning 而不是拒绝」,实际选的都是**静默**。
那句话已经在三个 Stage 里当过决策依据。

三条一起追补(不只 lang):

| code | 原来在哪 | 现在 |
|---|---|---|
| `phone_unparseable` | 只在日志 | 落库 + 名单页 |
| `no_contact_channel` | 只在日志 | 同上 |
| `cohort_tag_unknown` | 只在日志 | 同上,**带上那个拼错的 tag** |
| `no_default_cohort` | 只在日志 | 同上 |
| `lang_invalid`(新) | — | 同上,**带上收到的那个值** |

## 形状:jsonb 数组,每项 `{ code, context? }`

按你说的:一个 contact 可以同时 `phone_unparseable` + `lang_invalid` + `cohort_tag_unknown`,
单值会丢信息,而丢掉的那条恰好可能是要紧的那条。

`context` 是**「去哪改」那一格**([判断标准 9](#9-验一条失败路径要验的不只是它会失败还有它说的话够不够用来定位)):
`lang_invalid` 不带上 `EN`,运营只知道「有问题」,不知道 GHL 那边填了什么。

⚠️ **号码类的 context 只放形状,不放号码**(`12 chars, has letters`)——
warnings 会显示在名单页上、也可能被导出,而号码是 PII;完整原值在 `phone_raw` 里本来就有。
有一条用例钉这件事。

**每次覆盖不累加**:warnings 描述最近一次 payload 的状态。累加的话 GHL 那边修好之后
旧告警永远留着,而**一个永远亮着的告警等于没有告警**。

## RPC 那两个坑

### ① 参数变了就是新**重载**,不是替换

`create or replace function` 在参数列表变化时建的是新重载,旧的 8 参版本会留在库里。
那时 TypeScript 少传两个参数 → PostgREST 照样解析到旧重载 → **调用成功、
lang 与 warnings 静默不写**。所以迁移里先 `drop function`(带完整旧签名)再建新的。
一个「名字对、签名对不上」的入口比没有更危险。

### ② `lang` 不能无条件覆盖 —— 会撤销一个用户动作

学员在报告页切成英文之后那一列是 `en`。而 GHL 重复触发是常态(重发、重复付款回调、
workflow 配置失误),那些 payload 大多不带 lang。
若照 `coalesce(p_lang,'zh')` 无条件覆盖,他自己选的 `en` 会被**打回 `zh`** ——
一个用户动作被一次无关的重试静默撤销,而他只会发现「怎么又变中文了」。

所以 update 分支写 `lang = coalesce(p_lang, assessment_entitlements.lang)`:
**用库里的现值兜底,而不是用默认值兜底**。
调用方在「没给」与「给了但不合法」两种情况下都传 null,所以 null 的含义是
「这次不改语言」,不是「设成中文」。

## webhook 的 lang 入口:回落 + warning,不 400

理由已在上一节记过。补上你说的那一层,它比我给的三条更直接:

> **400 会把一个语言问题升级成一个付款问题。** 语言错了是体验问题,能补发;
> 付款进不来是收入问题,而且症状出现在最不该出问题的地方。
> 用严重后果去防轻微后果,方向反了。

## 名单页那一格

- **显示条数,不是图标。** 图标每天都在那里,看它一眼得不到可行动的信息,一周之后
  没人再看它;**条数是个会变的数字,变了人会注意到**。
- 点开走**已有的那个展开行**(与 `pdf_last_error` 共用):告警文本要整行铺开、要能复制,
  而单元格宽度不该被最坏情况决定 —— 与那次「错误文本横穿操作区」同一个教训。
- 筛选「只看有告警的」放在「只看号码解析失败的」**旁边**:两者是同一类东西
  (「这一行需要人看一眼」),运营的动作也是同一个。

## 反向验证:四次变异

| 变异 | 红的用例 |
|---|---|
| 删掉一条 `warn.*` 文案 | **3 条**,含「WARNING_CODES 与 ui-strings 一一对应」 |
| context 不再渲染 | 「带上 context —— 那是去哪改的那一格」 |
| `parseWarnings` 遇到坏项就抛 | 「认不出的项被丢掉,不抛」 |
| 没告警也渲染按钮 | 反向锁 |

那条「一一对应」是这份里最要紧的:漏一条文案的表现是名单页显示成 key 本身,
而**一条看不懂的告警比没有告警更容易被忽略** —— 它看起来「已经在显示了」。

## 未做

- **三条迁移都未应用**(标签四列 / entitlement.lang / warnings + RPC)。
  RPC 那条含 `drop function`,**是一次生产 schema 变更**,你 db push 时留意顺序
- **五处读同一列**(magic link / 报告页 / PDF / 分享卡 / GHL 消息)—— 下一轮
- 名单页那一格**只在本地渲染断言里看过**,没在真实数据上看过

Node **342**(+8)/ Deno 215,十一道门全绿。
