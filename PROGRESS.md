...
## ⚠️ 读之前先知道这一件:**省掉第 0 条没有任何症状**

这份文档里最容易退化的东西不是某段代码,是[判断标准第 0 条](#0-造一个检测器时第一条断言是它能抓到那个促使它诞生的东西)——
**造一个检测器时,第一条断言是「它能抓到那个促使它诞生的东西」。**

它之所以会被省掉,不是因为难,是因为**省掉之后什么都不会发生**:

| 你会看到 | 而它并不说明 |
|---|---|
| 门是绿的 | 它对真实发生过的那个案例会不会红 |
| 构建过了 | 同上 |
| 变异也红了 | 变异验的是「**它对我构造的那个输入**会不会红」 |

**那两个问题可以完全不重合。** `check:i18n-parity` 就是标本:
它诞生的原因是 `offer_routing` 只有 `zh_cta`,而第一版**恰好漏掉了 offer_routing**,
同时对着别处报出 9 处 —— 那 9 处看起来完全像一份正常的收获。

而且它**省起来快、做起来慢**:造回起因要改代码、跑一次、再改回来,四五步;
跳过它只需要不做。

> 所以:**新加一道门时,第一件事不是跑变异,是把那个真实案例造回去,看它红。**
> 起因已经被修好了的话,就刻意改回去跑一遍 —— 这一步没有替代品。
...
1. **[判断标准](#判断标准--这个项目反复用到的二十一条从第-0-条起)** —— 二十一条(第 0 条排在最前:造检测器先验它抓不抓得到起因)。它们不是格言,是每一条都对应一次真实的返工。
   不读这节,后面很多设计会显得像洁癖。
2. **[十四道门](#十四道门--每一道都是撞出来的)** —— 构建链上的十四道守卫各自守什么、**因为踩了什么坑才加的**。
...
`LiveSlide` 里那几个 `vmin` 常量决定一切,现场发现太小就改常量,用手机开一次全屏即可确认。
判据不在代码里 —— 是**后排看不看得清**。

### B. 需要第一场 MasterClass 的真实数据

这四件的共同点:**改动都很小,但会改变已经发出去的报告的含义** ——
所以每一件都按[判断标准 19](#19-改动成本远小于影响面时先停下来问这是不是一次产品决定)走:
先问「这个改动会不会让已经交出去的东西变成错的」。

1. **cohort 基准线** —— 现在只有 1 个真实样本,`min_n_for_baseline` 以下不给百分比
   (那是刻意的:shares 在阈值以下**不出现在 payload 里**,前端因此无法渲)。
   第一场之后才知道真实分布长什么样。
2. **题目校准** —— 15 题的区分度。哪几题「全场一边倒」要看
   `cohort_dashboard` 的每题选项分布(那一块就是为课前扫这件事做的)。
3. **`action_library` 的语气** —— 「读起来像不像我会说的话」。这条只有本人能判。
4. **`cost_model` 的系数** —— 五条 `formula` 里那些假设值(30% 流失、8 个百分点等)。
   改系数会让**已发报告里的金额**变成另一个数字。

### C. 需要真实样本才值得重新看的一个判据

**`isPriorityMismatch` 用 `!== weakest[0]`,而不是「不在最弱两维里」。**

当初考虑过改成后者(选中 `weakest[1]` 的人方向基本是对的,判成「方向选错了」偏严),
**没改**的理由是:报告第 7 板块已经按 `weakest[0]` 高亮并**发出去了**,
改定义会让已发出的报告与后台对不上。折中是布尔判定不动、另给三分
(`aligned / second_weakest / mismatched`)。

有真实样本之后值得重新看一遍 —— 而**那时的依据是数据,不是猜测**:
看「选中 weakest[1] 的人」占多少、他们与「选了一个根本不弱的维度」的人在
S5/S6 的开放回答上有没有可分辨的差别。
要改的话是一次产品决定,而且要连「已发出的报告怎么处理」一起想。

### D. 一直挂着的(与真实场景无关,但没做)

- **真正的冷启动耗时仍未测准** —— 两次都约 16 秒,但都是热的。
  要在**新部署之后的第一次调用**才拿得到
...
# 判断标准 —— 这个项目反复用到的二十一条(从第 0 条起)

**每一条都对应一次真实的返工,不是格言。** 括号里是它第一次出现的地方。


### 0. 造一个检测器时,第一条断言是「它能抓到那个促使它诞生的东西」

**排在第 1 条之前,因为它更基本。** 变异验证问的是「它会不会红」;
这一条问的是「**它对着最初那个案例会不会红**」——
而一个抓不到起因的检测器,后面所有的变异验证都是在验一个方向错了的东西。

标本:`check:i18n-parity`。它诞生的原因是英文 PDF 第 5 页那一行中文 CTA
(`offer_routing` 只有 `zh_cta`)。而第一版**恰好漏掉了 offer_routing** ——
`counterpart('zh_cta')` 里两个链式 `replace` 互相抵消
(`zh_cta` → `en_cta` → 又变回 `zh_cta`),于是它认为「对应字段存在」。

它报了 9 处、都在别处,而**那 9 处看起来完全像一份正常的收获**。
救回来的是一个**外部的预期答案**:那份 PDF 已经指着 offer_routing 说「这里有问题」,
两者对不上才暴露。没有那个答案的话,「OK」不会引起任何怀疑。

所以操作上:**写完检测器,先把起因那个案例喂给它,确认它红。**
起因案例已经被修好了的话,就临时把它改回去跑一次(这次就是删掉一条 `en_cta` 再跑)。

**为什么这一条特别容易被省掉:它没有症状。**
省掉之后门照样绿、构建照样过、变异照样红 ——
因为变异问的是「它对**我构造的那个输入**会不会红」,而这一条问的是
「它对**真实发生过的那个案例**会不会红」。**两者可以完全不重合。**

再加上一层不对称:**省起来快、做起来慢。** 造回起因要改代码、跑一次、再改回来,
四五步;而跳过它只需要不做。所以它不会以「偷懒」的形式出现,
而会以「这次显然没问题」的形式出现。

推论:**这条也决定了「先立门再改」的顺序。** 门先立起来时起因还在,
那一遍全量跑天然就是「它抓得到起因」的证明;
而改完再补门,就必须刻意把已经修好的东西改回去 —— 那一步很容易被省掉。
...
推论五:**变异之后,先确认这个变异真的改变了行为。**

推论三说「断言可能无事可做」;这一条是它的**镜像**:**变异也可能无事可做** ——
而两者在报告里长得一模一样(都是「改了代码,测试还绿」)。

而这一条更难发现:断言无事可做时,那条断言至少还在,读代码能看出它声称在验什么。
变异无事可做时,报告上只有一行「改了代码,测试还绿」,
**而那个结论本身是错的** —— 它看起来像「测试覆盖不足」,实际是「你没改到任何行为」。
于是人会去补一条根本不需要的断言,而真正的覆盖空缺仍然在那里。

标本:`downloadableIn` 那次。我加了一条 `?? 任意 ready` 的退路(意图是模拟
「递错语言的 PDF」这个危险行为),13 条全绿。而原因不是断言空 ——
`langStates` **总是**返回全部语言,所以 `find(s => s.lang === lang)` 永远命中,
那条 `??` 退路一次都走不到。换成真的会退回错语言的写法之后立刻红。

最简单的确认办法:**让变异版本在一个明确不同的输入上产出不同结果**。
产不出来,说明改的是死代码或不可达分支 —— 那时该问的不是「为什么没红」,
而是「这段代码是不是根本不会被走到」。

推论六:**变异证明「它会红」,还要有一次真实运行证明「它会绿」——
而后者不能用 fixture,必须用真实环境的输出。**

一道永远红的门和一道永远绿的门,危害不同但**成因一样**:都只验了一个方向。

标本:`check-migrations-applied` 那道部署 preflight。三次变异验的都是「它会拒绝」,
fail-closed 那条路也真跑过 —— 而**没有一次验过 happy path 在真实 CLI 上会放行**。
结果它在部署时红在了不该红的时候,人只能绕过它。
**被绕过一次的门,下一次就不会有人等它了。**

而 happy path 特别容易只用 fixture 验,因为 fixture 便宜:
喂一份「全部一致」的假输出,它当然绿。但门守的是**那个环境**,
而那个环境正是 fixture 唯一不覆盖的东西 ——
这条与推论五(观测环境要与运行环境一致)是同一件事的两侧。

操作上:守卫写完之后跑一次**真的**,把 exit 0 与那行 OK 输出记下来;
它红了就是它现在还不能上,而不是「等部署时再看」。

推论八:**两个人对同一条命令给出矛盾的观测时,先找环境差异,不要先判断谁记错了。**

> **观测者的环境是观测的一部分。**

标本:`supabase migration list`。我在脚本里(stdout 是管道)三次拿到 JSON;
对方在终端里手敲拿到表格。于是「你那条观测是错的」这个结论被提出来 ——
而**两个观测都是真的**,冲突只在我那句概括里(我把管道下的行为写成了这条命令的行为)。
真因是 **TTY**,而它**不在命令本身里**。

由此得到一条更常用的:**「我手敲看到 X」推不出「脚本会拿到 X」。**
凡是从人工试跑推导脚本行为的地方,都要先确认这一点 ——
CLI 给人看的是表格、给管道的是 JSON;`git` 的分页器、颜色、`--porcelain`
都是同一族。而脚本恰恰跑在那个「不给人看」的模式里。

操作上:两个观测对不上时,第一个问题不是「谁记错了」,而是
**「我们是不是在看同一个东西」** —— 列一遍双方的环境差(TTY、cwd、shell、
登录态、版本),差异往往就在里面。

推论九:**一条多层链路上,「修好了一层」与「链路通了」之间没有蕴含关系。**

而「看最终结果」这种验证方式只能告诉你**链路上有一处断**,
**不能告诉你是哪一处** —— 于是每一轮都会觉得「这次通了」,因为修的那一层确实通了。

标本:分享卡的语言错了**三次**,每次都在**上一次修的那层之外**:

| 第几次 | 断在哪一层 | 修完之后的感觉 |
|---|---|---|
| ① | `render-pdf` 硬编码 `lang=zh` | 「链路通了」 |
| ② | 报告端点 `currentLang` 只读 `entitlement.lang` | 「这次真通了」 |
| ③ | **前端请求根本不带 `lang`** —— 所以②一次都没被走到 | —— |

②那次尤其像完成了:**代码改对了、测试绿了、部署了,唯一缺的是没有人调用那条路。**
由此得到一句更短的:
...
1. **逐层验** —— 每一层各自给出一个可观测的判据(这一层收到了什么、传出了什么);
2. **或者接受「每次只能排除一层」** —— 那时不要说「修好了」,要说「排除了 ①」。

而分层的实际收益不在「这次修对了」,在**下次出错时搜索空间更小**:
「选哪一张卡」抽成纯函数并有断言之后,下一次要查的只剩「lang 有没有到 `cardsFor`」
这一段接线,而不是整条链。
...
推论三:**新守卫第一次全量跑,产出的是一份「现存问题清单」,而那份清单比人工搜索完整。**

上面两条讲的是「红的是谁」;这一条讲的是**用途**:门不只防回归,它先给你一份工作清单。
而它比人找得准,原因很朴素 —— **它按机器判据扫,人按记忆搜。**

标本:`check:legacy-columns` 立起来跑第一遍,报出 **9 处**读点。
我在写它之前手找过一轮,只想到 `render-pdf` 与 Admin 那两处;
剩下 7 处(`score` select 了却从没读值、`auth` / `seed` 在**写**一个没人读的列、
`quiz` / `report` 把它返回给前端)都是门报出来的。

推论:**遇到「有 N 处要改,但不确定 N 是多少」的任务 —— 先写判据。**
顺序是「立门 → 拿清单 → 改 → 门变绿」,而不是「改完再想要不要加门」。
后者会漏掉那些**你想不起来的**处,而漏掉的那几处恰恰是最危险的:
它们之所以想不起来,正是因为平时没人看那段代码。

而判据写宽了就会误报(那道门自己误报过两次:注释里的词、payload 的键名)。
处理方式是**收紧判据,不是加白名单** ——
白名单会让判据永远宽着,而白名单越长越没人看。
...
推论:**检查「某个维度覆盖全不全」时,遍历已有的条目找不到缺失的那些 ——
要从「按规则应该有哪些」那一侧列举。**

标本:英文 PDF 第 5 页出现一行中文 CTA。`offer_routing` 的五条只有 `zh_cta`,
**没有 `en_cta`** —— 而 `grep en` 找不到它:**那个键根本不存在,所以搜 en 搜不到它。**
遍历已有字段只能看见「已有的」。

从规则那一侧列举(「凡是 `zh*` 都该有对应的 `en*`」)之后,一次扫出 **19 处**,
分三族:`offer_routing.*.zh_cta`(5,渲染出来的)、
`cost_model.rules[*].zh_note`(5)、`report_sections[*].zh`(9)。
后两族**代码里没有任何读者** —— 它们是配置内部的说明,不是文案。

而这一条与[判断标准 11 推论三](#11-新标准--新守卫上线先对现有代码全量跑一遍--第一批命中的往往是自己刚写的)
是同一件事的两侧:那条说「门第一次全量跑给的是一份现存问题清单」,
这条说**那份清单必须由规则生成,不能由记忆生成**。

### 顺带:给一个已知缺陷造检测器时,先断言它能抓到那个缺陷

我为上面那件事写的门,第一版**恰好漏掉了 `offer_routing`** ——
`counterpart('zh_cta')` 里两个链式 `replace` 互相抵消
(`zh_cta` → `en_cta` → 又变回 `zh_cta`),于是它认为「对应字段存在」。

它被发现只因为我手上有一个**独立的预期答案**(那份英文 PDF 里的中文 CTA):
门报了 9 处而其中没有 offer_routing,两者对不上。
**没有那个已知答案的话,「OK」看起来完全正常** ——
一道为已知缺陷而生的门,却漏掉那个缺陷,是这一类工作里最容易发生的失败。

所以:**造检测器时,第一条断言是「它能抓到那个促使它诞生的东西」。**
...
推论六:**问「这个东西存不存在」时,用一个会报错的观测,而不是一个永远成功的观测。**

标本:确认 `assessment_entitlements.warnings` 这一列在不在。

| 观测方式 | 列不存在时 | 问题 |
|---|---|---|
| `select count(*) from information_schema.columns where column_name='warnings'` | 回 **0** | **永远成功**。0 还是 1 的含义全靠人记得 —— 而记错的方向恰好是「0 = 没问题」 |
| `select warnings from assessment_entitlements limit 1` | **报 42703** | 报错比 0 更难误读 |

那次它恰好没有把人带偏(0 确认了列不存在,与推理一致),但那是运气:
**一个永远成功的查询,它的输出需要被解释;而解释是记忆,记忆会错。**
报错不需要解释 —— 它本身就是结论。

同一族的还有:`grep` 找不到时退出码 1(可读)vs `grep -c` 回 0(要解释);
`curl` 不带 `-f` 时 404 也算成功(判断标准 17 那次)。

推论七:**任何以「放宽」为目的的 flag,要先问它放宽的是哪个环境的判断 ——
如果不是所有环境,那它掩盖的正是环境之间的差异。**

标本:`deno check --sloppy-imports`。加它的理由看起来很正当
(`api/_lib` 内部按约定写 `.js`,而磁盘上只有 `.ts`),而它确实让本地绿了。
代价是:

| 检查 | 结果 | 它验的是什么 |
|---|---|---|
| `deno check` | 绿 | 被 `--sloppy-imports` 放宽了 |
| `check:api-imports` | 绿 | `.js` 在 **Vercel** 运行时存在 |
| `supabase functions deploy` | **红** | Supabase 的打包器要磁盘上真实的 `.ts` |

**三个检查各自都对,而没有一个覆盖「Supabase 打包」这条路径** ——
而那条路径上的失败要等 deploy 才出现。

这与 `grep && commit` 那次(判断标准 17)是同族:**在门外面套一层让它不响。**
不同的是这次用的是**官方 flag、有正当用途**,所以更难识别 ——
「它让本地绿了」听起来像修好了,实际是把一个跨环境的事实降级成了本地的沉默。

替代做法是**把那个差异写成数据**:`supabase/functions/deno.json` 的 `imports` 里
两条相对路径重映射(`../../api/_lib/lang.js` → `.ts`)。区别在三点 ——
① 具体(点名两个文件,不放宽任何别的解析)② 声明式(映射是数据,配置里看得见)
③ 仍然受门校验(`check:dep-sync` 现在验它「目标存在 + 真的有人 import」)。
...
推论:**一个只输出、不改变控制流的检查,和没有检查的差别只在于人有没有认真看那一行。**

`grep … && commit` 那次是「在门外面套一层让它不响」;这一条是它的近邻 ——
**门响了,但它没有拒绝的能力。**

标本(同一个错误的第四次):我在编辑之前跑了 `git status -sb`,
它打印了 `## main...origin/main` —— 而我用 `&&` 把它和编辑串在一起,
于是那句输出**只是输出**,编辑照常执行。
前三次是「没检查」,这次是「检查了但它拦不住」,而后者更难自查:
日志里有那一行,看起来像做过检查。

判据很朴素:**这个检查有没有可能让下一步不发生?**
没有的话,它是提示,不是门。而提示的可靠性等于人的注意力。

处理方式:要么把它放到能阻断的位置(退出码 + `&&` 的**左边**,而不是右边),
要么做成钩子 / 门。这次的做法是 `.githooks/pre-commit` +
`scripts/assert-not-main.mjs`(判定抽出去才能测两个方向 ——
钩子自己没法「假装在 main 上」)。
...
推论四:**分支之间的依赖不在 git 里,只在人的记忆里** —— 这是「没人引用的入口」的分支版本。

`A` 建在 `B` 上意味着「合 `A` 就会带上 `B`」,但**没有任何东西提醒「`B` 单独没被合过」**。
于是一条分支可以每一步都对(建、写、push、说「它还没合」),而最后一步没人负责。

标本:`feat/entitlement-warnings` 装着 warnings 迁移**和** webhook 的 10 参调用。
我把后面每条分支都建在它的**父分支**上,于是它不在任何人的祖先里 ——
而一条不在任何人祖先里的分支,没有任何理由会被合。

它藏了两层:webhook 与迁移**一起缺**,所以彼此一致、系统看起来正常。
真正丢掉的是 `lang` 与 `warnings` 一直没被写 —— 又是一次静默。
...
合并前跑一次,空输出才是「都合过了」。
(以及:新分支要建在**那条还没合的分支**上,而不是它的父分支 ——
那样它就自动进入了必经路径。)

而这一族失败的机械对策不止 git 层面:代码与 schema 的契约可以静态检查,
`check:rpc-contract` 就是这次那个具体形状的门。
...
推论二:**比「关于未来的断言」更近一层的是「关于当下的警告」——
而写警告的人,就是要遵守它的那个人。**

标本:报告端点里那句
「PDF 渲染器也走这个端点,所以这一处读错会让渲出来的那份语言不对」——
**我写下那句话,然后在同一个函数里实现了它警告的那件事**
(`currentLang` 只读 `entitlement.lang`,不读 `?lang=`)。

与 `build:ci`、与 `syncToGhl` 那句「将来 tags 会自动被覆盖」同族,但更贴近:
前两个的验证对象在未来,而这一个**当场就可以验**。
写警告的动作让人产生「已经处理了」的感觉,而那种感觉恰好替代了处理。

判据:注释里写下「这一处如果 X 就会 Y」的那一刻,立刻问 ——
**这一处现在是 X 吗?** 答不上来就当成是,去验一次。
**这些都是一行改动、全库影响。**
...
`npm run build` 依次跑这十四道,**纯 Node,所以 Vercel 部署也跑同一条链**。
`npm run verify` 在这之上加需要 deno 的三项。

> **读这一节的目的**:没有「因为什么才加」这一列,这些门看起来像洁癖,
> 于是下一个人遇到红灯时的第一反应会是绕过它。它们没有一道是预防性的 ——
> **十四道全部是事后补的**,每一道对应一次已经付出的代价。

**十四道里有六道是某次真实事故的机械化**:`check:api-imports`(线上
...
还不存在的重载)。再加一道部署 preflight(`db push` / `deploy` 顺序反了)与一个
pre-commit 钩子(同一个错误四次)。

**所以这些门不是设计出来的,是长出来的** —— 每一道背后都有一个**具体的标本**。
而那也正是[第 0 条](#0-造一个检测器时第一条断言是它能抓到那个促使它诞生的东西)重要的原因:
**一道没有标本的门,连「它抓得到什么」这个问题都无从回答。**

### ⚠️ 一道门的价值会随代码变化而变化,而它自己不知道

标本:`check:legacy-columns`。它诞生时守的是「读了第二个真相源」——
旧列还在库里,读它只是拿到一份可能过期的数据。
而 `20260813000000` 把那些列删掉之后,读它 = **PostgREST 42703 → 端点 500**。

**判据一个字都没改,而它拦住的后果严重了一档。**
没有任何机制记录了这次升级 —— 只有那道门文件里被手写进去的一句注释。

推论:**门的注释里要写「它现在守的是什么后果」,而不只是「它当初为什么被加」。**
后者是历史,前者会变;而变了之后没人重新评估的话,
下一个人可能因为「这条列已经删了」就把它撤掉 ——
而那些列名在新表里**依然同名存在**,所以那个错误现在只会更严重。

### 而「后果」本身也应该是推出来的,不是手写的

上一版把那句「语义变强了」**手写**进了门的注释 —— 而手写的东西要人维护,
正是这个项目一路在拆的那一类。

现在 `check:legacy-columns` 自己算:扫迁移里的 `drop column`,
命中就说「这些列**已经从库里删掉了**,读它 = 42703 → 端点 500」,
否则说「旧列还在,读它是第二真相源」。

【为什么不连库去查】考虑过,**否掉了**,两条理由:

1. **构建时没有库**,而这正是 `check:rpc-contract`(守仓库内部一致性、并明说守不了
   「迁移跑过没有」)与部署 preflight(连得上库)分工的前提。给构建门加库依赖,
   要么 Vercel 构建拿不到凭证而红,要么连不上时 fail-open ——
   后者正是 `--sloppy-imports` 的形状:**一道在某些环境里不响的检查等于没有检查**。
2. **推出来的答案不改变这道门的动作** —— 两种情况都是拒绝,只有措辞不同。
   为一个字符串付一个库依赖,交换不划算。

而这件事**本来就在仓库里**:`drop column` 写在迁移文件中,而迁移是这个仓库里
schema 的真相源。所以推得出来,而且不用付上面两项代价。

由此得到一条更一般的:**要推导一个事实时,先找那个「记录了这次变化的产物」,
而不是去问运行中的系统。** 前者在仓库里、离线可读、不需要凭证;
后者需要连接、会因环境而不同(TTY 那次就是),而且拿不到时容易被写成 fail-open。

三个方向都验过:两组列都已删 → 说 42703;把那条 drop 迁移移走 → 措辞退回「第二真相源」;
混合(一列已删一列还在)→ 分开说。
...
| `check:legacy-columns` | 已经搬走的列不许再被读:`assessment_results.pdf_*`(→ `assessment_report_files`)、`assessment_sessions.locale`(→ `assessment_entitlements.lang`);例外要在代码里写理由(≥12 字符)| **删列不可逆,所以搬家分两步**,而中间那段时间旧列仍然可读。关键那句是:**一列没人读就不构成第二个真相源 —— 危险的是读它的人,不是列本身**。所以顺序是「门先立起来,列之后再删」。这道门跑第一遍就报出 **9 处**读点,其中 7 处当场改掉了 —— 它同时是审计工具 |
...
- **`check:deno`** 拦下了 `assessment-admin` 里那份**自己的** `RosterRow` 类型 ——
  ...
  这正是「[verify 靠人记得跑](#十四道门--每一道都是撞出来的)」那个已知洞会漏掉的东西。
...
而 PROGRESS 里[早就写着](#十四道门--每一道都是撞出来的)「加人之后上 GitHub Actions 跑 `npm run verify`」。
...
# 语言 wiring:render-pdf / sweep / 学员端点

## 学员端点用 **cookie**,不是 renderToken —— 与最初设想不同

设想是「用 renderToken,跟报告页取数同一把」。查证之后:`assessment-report` 接受**两条**入口
(cookie 或 `?rt=`),而**学员在浏览器里能用的只有 cookie** ——
`rt` 只出现在 PDF 渲染器和 Admin 的 `report_link` 那两条路径上。
若新端点只认 rt,**真实学员一次都点不动那个按钮**。

而 renderToken 想要的那个保证(「只能触发自己那份」)cookie 同样给得出:
**session 从 cookie 来,而不是从参数来** —— 那正是三条约定里的第 2 条。
另外它刻意**不接受 `?rt=`**:Admin 走自己的 action,两条路径的鉴权与留痕从一开始就分开。

三条约定都落了:只接受 `lang`(session 从 cookie)、幂等
(`ready` 直接返回不重渲 / `working` 返回正在生成 / `exhausted` 回 409 并说要人介入)。

【lang 不合法在这里回 400,与 webhook 那个决定方向相反,而且是有意的】
webhook 是付款入口,拒收的代价是「客户付了钱系统里没有他」;
这个端点只被我们自己的报告页调用,一个不合法的 lang 意味着**我们自己的代码写错了** ——
静默回落只会让那个 bug 活得更久。

## `render-pdf`:`lang` 必填,而且**从这一行读,不从 entitlement 读**

两处硬编码的 `lang=zh` 换成参数。而参数的来源是关键 ——

> 从 entitlement 读的话,学员切到英文之后触发重渲中文版,会渲出**英文的中文版**:
> 文件名说 zh,内容是 en,而两者都不会报错。

所以 `lang` 是**必填、没有默认值**:这个端点只被三个自己的调用方打
(finalize / sweep / 学员端点),而一个默认值会把「调用方忘了传」变成「静默渲成中文」。
三个调用方都改了:finalize 传这个人的语言(第一次生成),sweep 传**那一行**的语言。

状态读写全部搬到 `assessment_report_files`,按 `(session_id, lang)`;
初始那次改成 **upsert** —— 学员点「生成英文版」时那一行还不存在,
而 `update` 在没有行时是静默的 0 行影响:状态永远停在「什么都没有」,
渲染却照常跑完写 ready,中间那段「正在生成」对前端不可见。

## sweep 的粒度换成 (session, lang)

按 session 挑的话同一份会被重复渲 —— 而那不是「多花一次 Lambda」:
两次渲染写同一个对象,后一次覆盖前一次,**可中间那一次的 attempts 已经记上了**。
`PdfSweepRow` 多一个 `lang`,陈旧判断的兜底列从 `computed_at`(旧表)换成 `created_at`(新表)。

## 撞上「三个运行时」那条规则的第一个真实案例

`reportFiles.ts` 是**第一个** 「`api/_lib` 内部互相导入、而 Deno 也要再导出」的模块。
`api/**` 的相对导入按约定写显式 `.js`(Vercel 的 Node ESM 语义),而磁盘上只有 `.ts`:

| 谁 | 反应 | 处理 |
|---|---|---|
| Deno 的 `check` | 解析不到 `./lang.js` | 加 `--sloppy-imports`(只放宽 Deno 侧的解析) |
| **`check:dep-sync`** | 「解析不到,**不当作通过**」 | **认这条约定** —— `.js` 也试一次 `.ts` |

第二条是这道门报得对:一个解析不到的 specifier 就是没走完 import 图。
缺的是那条映射,而不是门太严 ——
**两道门必须认同一套约定**(`check:api-imports` 早就这么解析),
否则同一份代码在一处合法、在另一处报错,而人会开始怀疑门而不是代码。
修完之后 dep-sync 自动发现的共享文件从原来的数量涨到 **88 个** —— import 图真的走深了。

⚠️ `--sloppy-imports` 是一次**真实的放宽**,边界写在这里:
它只影响 Deno 侧的解析;「那个 `.js` 在 Vercel 运行时真的存在」仍然由
`check:api-imports` 守着,而那道门守的正是出问题的那个运行时。

## 另一件查出来的:`assessment_sessions.locale` 是**第三个**语言来源

它存在、被 select、而**没有任何一处读它的值**(语言判定现在只认 `entitlement.lang`)。
那正是「名字像入口但没人引用」的形状。这一轮没删 —— 删列不可逆,而且要先确认
前端有没有间接依赖。已进未完成清单。

## 未做(下一轮)

- **Roster 那一格**(`zh ✓ / en —`)与**报告页两个按钮**(「下载 PDF(中文版)」+「生成英文版」)
- **`check:pdf-columns`**:禁止再读 `assessment_results.pdf_*` ——
  在它到位之前,旧列仍然是一个可读的第二真相源
- `assessment-admin` 的 `render_pdf` action 还没接 lang(它现在会被 render-pdf 400 拦下,
  **这是刻意的**:改坏了要立刻可见,而不是静默渲成中文)
- 五条迁移未应用

Node 347 / Deno 216,十一道门全绿。

# 第十二道门 `check:legacy-columns`,以及 Admin 的显式语言

## 门先立起来,列之后再删

两组列已经有新家,但旧列还在库里(删列不可逆,分两步):
`assessment_results.pdf_*` → `assessment_report_files`;
`assessment_sessions.locale` → `assessment_entitlements.lang`。

关键那句:**一列没人读就不构成第二个真相源;危险的是读它的人,不是列本身。**
门在的那一刻旧列就已经不是真相源了,删列只是收拾残留。

### 这道门跑第一遍就把这一轮的活儿列全了:9 处

它同时是**审计工具** —— 比我手找准。9 处里 7 处当场改掉:

| 处 | 改法 |
|---|---|
| `assessment-score` select 了 `locale` 却从没读值 | 从 select 与类型里删掉 |
| `assessment-auth` **写** `locale: lang` | 不再写 —— 那一列 `not null default 'zh'`,不传就是让默认值填;**写一个没人读的列只会让「哪个才是真的」一直存在** |
| `seed-test-data` 写 `locale: 'zh'` | 同上 |
| `assessment-quiz` 把 `session.locale` 返回给前端 | 改成嵌 `entitlement.lang`;**payload 的键名仍叫 `locale`**(不牵动前端),换的是**值的来源** |
| `assessment-report` 的两处 select + payload | 同上 |
| `assessment-admin` 的 `render_pdf` | 搬到新表 + 显式 lang(见下) |
| **`assessment-report` 的 pdf 那几列** | **没改,带理由豁免** —— 见下 |

### 唯一的豁免,以及为什么这次不改

报告页的下载按钮还在读旧列,而那条路径是**学员面向**的。
半改会让**生产上学员的下载按钮坏掉** —— 而这和 Admin 那个按钮坏掉不是一回事:
后者我明说了、影响一个人的一次操作;前者影响每一个拿到报告的客户。

豁免带着改法一起写在代码里(读全部语言行、payload 回 `[{ lang, availability, url }]`、
前端渲两个按钮),而**这道门每次构建都会把接受的豁免打印出来** ——
所以它不会安静地变成永久状态。
...
| 第几版 | 误报什么 | 修法 |
|---|---|---|
| 一 | 报了**它自己那几条解释性注释**(「不再写 `locale`…」这句里就有那个词) | 扫之前把注释挖空,**保留换行**以便行号仍然对得上 |
| 二 | 报了 `return { locale: effectiveLang(...) }` 这个**payload 键名** | 窗口从「后面 1200 字符」收紧成「只取 select / insert / update / upsert 的参数」 |

两次都是同一件事:**一道误报的门会让人开始习惯性忽略它,而那正是它失效的方式。**
新表用的是**同名列**(`pdf_status` / `pdf_path` …),`locale` 在前端还是 i18n 的正当叫法 ——
所以判据必须是「表 + 列 + 出现在查询参数里」,只按名字扫一定误报。

三次变异:把 `locale` 加回 score 的 select → 红;去掉那个豁免标记 → 红;
把豁免理由改成 `ok` → 红(「至少 12 个字符」)。

## Admin 的 `render_pdf`:**显式选语言,没有默认**
...
名单页那一格显示的是 `zh ✓ / en —`,所以「他在看哪一份」界面上本来就是分开的,参数也该分开。
重置那次写入改成 **upsert** —— Admin 也可以为一种从没生成过的语言点「生成」。

## 未做(下一轮,按顺序)

1. **`assessment-report` 的 pdf 读点**(那个豁免)+ **报告页两个按钮**
   (「下载 PDF(中文版)」/「生成英文版」—— 文字不用图标,图标会被眼睛过滤掉)
2. **Roster 那一格** `zh ✓ / en —`
3. 旧列的删除迁移(等生产跑通一段)

Node 347 / Deno 216,十二道门全绿。

# 报告页两个按钮:那个豁免消失了

## 唯一的豁免已经不在

上一轮 `check:legacy-columns` 只剩一处豁免(报告页还在读旧的 `pdf_*` 列)。
这一轮把它改掉了,门现在是 **「2 组已搬走的列没有任何读点(无豁免)」**。

`assessment-report` 改成读 `assessment_report_files` 的**全部语言行**,payload 回:

```
files: [{ lang, availability, url, cardUrl, cardTallUrl }]
```

【为什么整个数组都回】那两个按钮里「生成英文版」的存在与否取决于**另一种语言**的状态。
只回当前那份的话,那个按钮该不该出现就无从判断。

### 兼容投影,而不是第二个来源

`pdfStatus` / `pdfUrl` / `cardUrl` / `cardTallUrl` 四个键留着,但**从 `files` 派生** ——
同一份数据的两个视图。

留着的理由很具体:部署是前后端一起上,**而用户的浏览器里可能还挂着上一版的 JS**
(报告页一开就是几十分钟)。删掉那四个键会让那些页面的下载按钮**当场消失**,
而这是学员面向的路径。等一个版本之后再删。

## 两个按钮,不是一个下拉;写语言名,不用图标

抽成 `ReportFileActions`(与 `RosterRow` / `ShareCardView` / `LiveSlide` 同一个先例)——
抽出来之后这段 JSX 才能被 `renderToStaticMarkup` 直接渲,断言才落在真实按钮上
(`Report.tsx` 自己在 SSR 下只有 loading 态)。

| 状态 | 给什么 |
|---|---|
| 当前语言 ready | 「下载 PDF(中文版)」**primary** |
| 另一种语言 ready | 「下载 PDF(英文版)」outline —— 他要的那份才是主动作 |
| `absent` / `failed` | 「生成英文版」**显式动作**,他知道自己在触发一次生成 |
| `working` | 文案,**不给按钮** —— 给按钮等于邀请他在系统已经在做的事上再点一次 |
| `exhausted` | 说明 + 打印兜底,**不给一个必然失败的按钮** |
| 当前语言还在渲(finalize 刚触发) | 文案;轮询放弃之后才给按钮 —— 那时「一直没好」是事实而不是猜测 |

【点击那一刻重新取一次】signed URL 只活 1 小时而报告页一开就是几十分钟。
点击时重取把「URL 会过期」从错误处理变成不存在的问题 —— 与原来那条同一个取向,
只是现在要按语言取。
...
| 按钮文案不带语言名 | **3 条** |
| `working` 也给按钮 | 2 条 |
| `exhausted` 给一个必然失败的按钮 | 1 条 |
| 当前语言正在渲时也给按钮 | 1 条 |
| 两个按钮同等强调 | 1 条 |

另外用构建产物的 CSS 渲了四种状态在浏览器里看过(最常见的那种、两种都 ready、
英文正在生成、英文用完重试)—— 与[判断标准 10](#10-用-css-表达的意图没有渲染断言就要假设它没生效)
同一个要求:结构断言之外还要看一眼实际渲出来的样子。

## 未做(下一轮)

1. **Roster 那一格** `zh ✓ / en —` —— 做完它 Admin 的「重新生成」才能传 lang(那个按钮
   现在会 400,已经坏了两版,**每一版都明说过**)
2. 旧列的删除迁移(等生产跑通一段)

Node **357**(+10)/ Deno 216,十二道门全绿。

# `feat/entitlement-warnings` 从没被合 —— 以及它的机械对策

## 先说结论:付款 webhook **没有坏**

| 事实 | 状态 |
|---|---|
| main 上的 webhook | **8 参**(`p_lang` / `p_warnings` 都没有) |
| 库里的 RPC | 仍是 8 参(要 drop 它的正是那条没应用的迁移) |
| 结论 | 调用方与库**匹配**,webhook 正常 |

**代价是另一件,而且正是静默的那种**:`lang` 与 `warnings` 从来没被写过 ——
每条新记录的 `lang` 停在默认 `'zh'`,warnings 只在函数日志里。
也就是我们上上轮刚说要治的那个静默,它以另一种形式成立了。

## 根因:一条不在任何人祖先里的分支

`feat/entitlement-warnings` 是当时**唯一未合**的分支,而后面每一条我都建在它的
**父分支**(`feat/lang-on-entitlement`)上 —— 于是它被绕过了。
每一步都对(建、写、push、末尾还说了「它还没合」),只是最后一步没人负责。

已记进[判断标准 18 推论四](#18-死代码的危险不是占地方是它可能在你不知道的时候被当成活的--名字越像正规入口的越危险)。
不依赖记忆的观测是 `git branch --no-merged main`;
而更根本的做法是**新分支建在那条还没合的分支上**,让它自动进入必经路径 ——
这一轮的 `feat/rpc-contract-gate` 就是这么建的。

## 顺序问题:成立,理由是具体的

`200000`(warnings + RPC)晚于 `300000`(report_files)应用**没有问题**,
因为两者**没有交集**:`300000` 里一处都没提到 `upsert_assessment_entitlement` /
`warnings` / `assessment_entitlements`,而 `200000` 只碰那一列和那个函数。
`drop function` 的对象(8 参 RPC)也不被 `300000` 依赖。

## 第十三道门:把那个巧合换成事实

`check:rpc-contract` —— 代码传的参数必须与某条迁移里的最终签名逐个对上,两个方向都拦。

三次变异,**第一条就是重现这次的局面**:把那条迁移文件移走(等于它没被合进来)→
门立刻报「传了迁移里不存在的参数:`p_lang`, `p_warnings`」。另两条:多传一个参数、漏传一个参数。

⚠️ **它守不了什么,写在门里**:「那条迁移在生产库里跑过没有」需要连库才知道,
而构建时没有库。所以部署顺序(`db push` → `deploy:functions`)仍然靠人 ——
**不装作守得住**。

## 那个 `count(*) = 0` 的观测

不是新事实(列确实不存在,与推理一致),但它暴露了一个观测方式的问题,已记进
[判断标准 14 推论六](#14-有直接观测可用时先观测再推理):
**问「这东西存不存在」要用一个会报错的观测,而不是一个永远成功、输出需要被解释的观测。**
`information_schema` 那条永远成功,0 还是 1 全靠人记得;
`select warnings from …` 会报 42703,而报错不需要解释 —— 它本身就是结论。

## 顺带确认的两件

- **表的列名是 `pdf_status` / `pdf_path`**(不是 `status` / `path`)。这是故意的:
  沿用旧列名让 `render-pdf` 那几处只改表不改字段名。代价正好是
  `check:legacy-columns` 的判据必须带上表名
- 你已经把 `fix/merge-entitlement-warnings` 合进 main 并推了(reflog:fast-forward 到 `d514bae`),
  所以现在 `git branch --no-merged main` 是空的

Node 365 / Deno 216,十三道门全绿。

# Supabase 打包失败:同一个文件被两个打包器消费,而它们要求相反

## 诊断结论:**不是自由选择,而且有第三条路**

### 三个环境对同一个 specifier 的要求

| 环境 | 要求 | 证据 |
|---|---|---|
| Vercel(tsc + 运行时) | **必须 `.js`** | 写 `.ts` 会被 **tsc 自己**拦:`TS5097`(要开 `allowImportingTsExtensions`,而那个选项要求 `noEmit` —— 与 Vercel「要产出 JS」直接冲突) |
| Supabase 打包器 | **必须磁盘上真实的 `.ts`** | deploy 报 `Module not found ".../api/_lib/lang.js"` |
| `deno check`(本地) | 两者都能忍 | 因为上一轮加了 `--sloppy-imports` |

所以「改成 `.ts`」这条路**被 tsc 直接堵死**,不需要赌线上。实测过:改完 `.ts` 之后
`tsc -p tsconfig.api.json` 立刻 TS5097 ×2。

### 第三条路:把差异写成**数据**,而不是放宽某个环境的判断

`supabase/functions/deno.json` 的 `imports` 加两条**相对路径重映射**:

```json
"../../api/_lib/lang.js":     "../../api/_lib/lang.ts",
"../../api/_lib/pdfState.js": "../../api/_lib/pdfState.ts"
```

`--sloppy-imports` 同时摘掉了。区别在三点:
① **具体** —— 点名两个文件,不放宽任何别的解析;
② **声明式** —— 映射是数据,配置里看得见;
③ **仍然受门校验** —— `check:dep-sync` 现在验它「目标文件存在 + 真的有 api 文件在 import 那个 `.js`」。

⚠️ **这条路只在本地验过**(`deno check` 绿、`deno test` 绿)。
Supabase 的打包器读的是同一份 import map(每个函数都声明了 `import_map`),
所以**应该**成立 —— 但那句「应该」要等一次真实 deploy 才能变成事实。

## 为什么是 `reportFiles` 第一次撞上

`api/_lib` 下有 **7 个**模块被 Deno 侧再导出,其中 **6 个内部导入是 0** ——
`reportFiles` 是唯一有内部导入的(2 个)。以前每个共享模块都恰好是**叶子**,
那条 `.js` 约定从来不需要被 Deno 解析。

## 顺带修掉两个「门自己的缺陷」

**① `check:api-imports` 的提示是错的。** 它把「不以 `.js` 结尾」一律说成「没有扩展名」,
于是写了 `./lang.ts` 的人会看到「没有扩展名 —— 写成 `./lang.ts.js`」——
两句都不对,而后一句照做会得到一个不存在的路径。
**一道门的提示错了比不提示更糟:它把人推向一个更深的坑。**
现在分两种说法,并且 `.ts` 那条直接说出 TS5097 与 `noEmit` 的冲突。

**② `check:dep-sync` 报了新加的两条重映射「没人用」。** 它报得对 ——
它按「裸 specifier 有没有被 import」判断,而重映射是另一类条目。
处理方式是**扩展它的模型**而不是开白名单(白名单会让它「不受本守卫校验、静默腐烂」,
正是那段代码原本要防的事):相对路径条目有自己的判据 ——
目标文件必须存在,且那个 `.js` specifier 必须真的被某个 api 文件 import。两条各自变异验过。

### 而我在写②的时候又栽了一次「永远绿」

新代码里我写的是 `problems.push(...)`,而那个脚本里的累加器叫 `errors` ——
`problems` **根本不存在**。后果分两半:

- 正常路径:循环不 push,**门照常绿**;
- 该报错的路径:`ReferenceError: problems is not defined`,**直接崩**,而不是报出问题。

也就是说那段新判据在两种情况下都没有履行职责,而门是绿的。
它只被抓到是因为跑了变异**并且坚持看完整输出** ——
第一次跑变异时我 grep 了预期的措辞,拿到空输出,而空输出既能是「没触发」
也能是「崩了」([判断标准 14](#14-有直接观测可用时先观测再推理):
拿不到观测就不算观测)。

## 未做

- ✅ **那条 import map 重映射已被真实 deploy 验证**(10 个函数全部打包成功,含新的
  `assessment-report-file`)。所以「非叶子模块跨运行时共享」现在有**两条已知可行的路**:
  ① `deno.json` 的相对路径重映射(已验证,首选 —— 不改代码)
  ② 让模块变回叶子(把它依赖的常量做成参数),代价是调用方多传值。
  ②没用上,但留在这里 —— 下次再撞上时不必重新推一遍
- Roster 那一格 `zh ✓ / en —`(原计划下一轮)

Node 365 / Deno 216,十三道门全绿。

# 部署顺序做成机械的,以及 warnings 的 null / [] 之辨

## `warnings` 那一列只有**两个**状态,`[]` 不会出现

你看到的不是出入,是两个不同的东西:

| 位置 | 值 | 代码 |
|---|---|---|
| **响应体** | `[]` | `warnings: allWarnings`(第 205 行) |
| **那一列** | `null` | `p_warnings: allWarnings.length ? allWarnings : null`(第 175 行) |

所以这一列实际只有 `null`(没告警)与非空数组(有告警)两种,**`[]` 在列里根本不会出现** ——
「没写过」与「写过但没告警」在写入侧就是同一个值,不存在被读取侧混淆的问题。

### Roster 那一格不会崩,而且不是靠运气

两个调用点都经 `parseWarnings`,没有任何地方对原值取 `.length`:

```ts
export function parseWarnings(raw: unknown) {
  if (!Array.isArray(raw)) return [];   // null / undefined / 脏值 → []
```

而且**已经有一条用例钉着**(`null / 非数组 / 空数组一律回空`)。
seed 那 15 条与历史行的 `warnings` 是 null(迁移只加列不回填),
所以名单页第一次渲染时大部分行走的正是这条路 —— 它有断言,不靠「应该没问题」。

## 部署顺序:从「靠人记得」变成一道 preflight

你说得准:**那次没出事的原因是还没有真实学员,不是顺序不重要。**
「靠人记得」在没有流量时看起来一直有效,而它失效的那一次代价最大。

所以 `deploy:functions` 前面插了 `check-migrations-applied.mjs`:
跑 `supabase migration list`,**有任何一条本地迁移没应用到远端就拒绝部署**。

反方向也拦:远端有一条本地没有的迁移 —— 那说明本地少了文件,或者有人直接在库上改过。

### 【拿不到答案时一律拒绝 —— fail closed】

CLI 失败、没登录、没 link、网络不通,都**不能**当成「没有未应用的迁移」。
那会让这道检查在**最需要它的时候**(环境不对)自动让路 ——
而那正是 `--sloppy-imports` 那次的形状:一道在某些环境里不响的检查等于没有检查。

### 为什么不做成构建门

`npm run build` 时没有数据库。`check:rpc-contract` 守的是仓库内部一致性,
而它自己明说守不了「那条迁移跑过没有」。这件事**只能在部署时问**,而部署时恰好连得上库。
两道检查因此是互补的,不是重复的。

### 顺带一个反直觉的实测 —— **而我当时把结论说宽了,已更正**

原文写的是「不带参数输出 JSON,带 `--output json` 反而输出表格」。
那**两个观测都是真的**(管道下复现过三次),但**结论说宽了**:真正的决定因素是
**stdout 是不是 TTY** ——

| 怎么跑 | 输出 |
|---|---|
| 脚本里 `execFileSync`(stdout 是管道) | **一行 JSON** |
| 人在终端里手敲 | **表格** |
| 加 `--output json`(即使在管道下) | **表格**(实测两次) |

所以「人看到的」与「脚本拿到的」根本不是同一个东西 ——
而我用管道下的观测去描述这条命令的行为,那句话对脚本成立、对人不成立。

现在两种格式**都解析**,于是它不再依赖「哪一种才是真的」这个我不该赌的前提。

三次变异:有一条没应用(重现这次的顺序)/ 远端多一条 / CLI 输出格式变了 —— 各自红。

Node 365 / Deno 216,十三道门全绿 + 一道部署 preflight。

# 把「检查」换成「拒绝」:`.githooks/pre-commit`

## 同一个错误第四次,而这次形态不同

前三次是**没检查**;第四次我**跑了** `git status -sb`,它打印了 `## main...origin/main` ——
而我用 `&&` 把它和编辑串在一起,于是那句输出**只是输出**,编辑照常执行。

> **一个只输出、不改变控制流的检查,和没有检查的差别只在于人有没有认真看那一行。**

已记进[判断标准 17](#17-用-grep-判断状态之前先问匹配成功意味着什么-它常常和你要的答案相反) 的推论 ——
它与 `grep … && commit` 是近邻:那次是「在门外面套一层让它不响」,
这次是「门响了,但它没有拒绝的能力」。而后者更难自查:日志里有那一行,
**看起来像做过检查**。

## 做法:钩子 + 可测的判定

`.githooks/pre-commit` 调 `scripts/assert-not-main.mjs`:
在 `main` 上且**不是**合并提交就退出 1。

- **放过合并提交**:这个仓库的工作方式就是「把分支合进 main」,而带冲突的合并要
  `git commit` 收尾。一律拒绝会拦住正常工作流,而**一道拦住正常操作的门会被关掉**。
- **判定抽成脚本才测得了两个方向**:钩子跑的时候 HEAD 就是待判的分支,
  没法「假装在 main 上」。而一道没验过红路径的门正是上一条教训本身。

四个方向都验过:`main` 非合并 → **拒绝**;`main` 合并中 → 放行;
别的分支 → 放行;**真实当前分支 → exit 0**。

### 启用要一次性配置(而且它现在还保护不了 main)

```bash
git config core.hooksPath .githooks
```

⚠️ 钩子文件是**版本控制里的**,所以它只在「已经进了 main」之后才保护 main 上的提交 ——
在那之前 checkout 到 main 时那个文件根本不存在。
我在验它的时候正好踩到这一点:`git stash -u` 把 `.githooks/` 一起藏走,
于是切到 main 时钩子不存在、探针提交**通过了**(那个提交已复位,`main` 回到 `origin/main`)。

## 顺带把两条观测教训记下

- [判断标准 14 推论八](#14-有直接观测可用时先观测再推理):
  **两个人对同一条命令给出矛盾的观测时,先找环境差异,不要先判断谁记错了** ——
  观测者的环境是观测的一部分。由此得到更常用的一条:
  **「我手敲看到 X」推不出「脚本会拿到 X」**(TTY / 分页器 / `--porcelain` 都是同一族)
- 那条 `supabase migration list` 的更正已经写在它自己那一节里:两个观测都真,
  错在我那句概括

## Admin 那个按钮:坏了三版,现在能用了

生产上 `f12bab5f` 有 zh 与 en 两行、都 ready —— 而每一版坏着的时候都明说过。

Node 367 / Deno 216,十三道门全绿。

# 英文 PDF 暴露的两处缺口:一处是真 bug,一处不止一族

## 第 2 条(分享卡)是真 bug,而**不在分享卡那条路上**

`render-pdf` 两处 `goto` **都传了** `&lang=${lang}`(main 上也是,查证过)。
问题在报告端点:`currentLang` **只从 `entitlement.lang` 算,从不读 `?lang=`**。

| 层 | 用的哪个语言 |
|---|---|
| 页面正文 | `?lang=en` → **英文**(前端 i18n 读 query) |
| payload 的 `cardUrl` | `entitlement.lang` = `zh` → **中文卡** |

最常见的触发方式:运营在 Admin 点「重新生成 en」,而**学员自己的语言还是 zh** ——
渲的是那一份文件,读的是这个人的偏好,两者本来就可以不同。

**而我在下面兼容投影那段的注释里写着**「PDF 渲染器也走这个端点,所以这一处读错会让
渲出来的那份语言不对」—— 写了警告,然后实现了它警告的那件事。

修法一行:`effectiveLang(stored, incoming)` 本来就是为这件事写的 —— 合法的 incoming 赢。
`lang_test.ts` 补了一条钉「渲染器传来的 `?lang=` 必须赢」,变异(让 stored 赢)→ 3 条红。

⚠️ **端点里那一行接线本身没有断言** —— 去掉那个参数后 `deno check` 照样绿。
与其他 Edge Function 入口同类:本地无处可测,真正的判据是**下一次英文 PDF 里那张卡**。

## 第 1 条(单语字段):`offer_routing` **不是唯一的**,一共 19 处三族

按「应该有哪些」那一侧列举(不是 grep en),一次扫出:

| 族 | 处数 | 有人读吗 |
|---|---|---|
| `offer_routing.*.zh_cta` | 5 | ✅ **渲染出来的** —— 就是 PDF 第 5 页那一行 |
| `cost_model.rules[*].zh_note` | 5 | ❌ 代码里没有读者 |
| `report_sections[*].zh` | 9 | ❌ 代码里没有读者(是配置内部的说明,不是文案) |

所以要写的英文文案只有 **5 条 `en_cta`**;后两族是配置里的内部说明,
**逼它们双语只会让人开始忽略这道门**。

### 这道门刻意**没有**进 `build`

19 处里 10 处是内部说明,现在进构建链会让 `build` 立刻红,而红的原因里
大部分不该修 —— 一道误报的门会被习惯性忽略。
所以先作为 `npm run check:i18n-parity` 存在;等 5 条 `en_cta` 落地、
并且给那两族定下豁免方式(JSON 里放一个带理由的兄弟键,而不是在门里开白名单),
再收进 `build`。**这个顺序是刻意的,不是忘了。**

### ⚠️ 而这道门的第一版恰好漏掉了 `offer_routing`

`counterpart('zh_cta')` 里两个链式 `replace` 互相抵消(`zh_cta` → `en_cta` → 又变回),
于是它认为「对应字段存在」。**一道为已知缺陷而生的门,却漏掉那个缺陷。**

它被发现只因为我手上有一个**独立的预期答案** —— 门报了 9 处而其中没有 offer_routing,
与那份英文 PDF 对不上。没有那个已知答案的话,「OK」看起来完全正常。
已记进[判断标准 12](#12-覆盖范围是手写的守卫会被代码悄悄长到边界外面--让覆盖由运行时事实驱动) 的推论:
**造检测器时,第一条断言是「它能抓到那个促使它诞生的东西」。**

## 这次的验证方式本身

「那份 PDF 会直接告诉你哪些地方还是中文」—— 它确实告诉了,
但它告诉的不是「翻译漏了」,而是**结构上没有 en 的位置**。
`grep en` 找不到这一类,因为那个键根本不存在。**只有真渲一次英文版才看得见。**

Node 367 / Deno **217**(+1),十三道门全绿(`check:i18n-parity` 暂不在其中)。

# 五条 `en_cta` + 第十四道门收进构建链

## 三件一起做,因为它们互为前提

| 做了什么 | 为什么不能单独做 |
|---|---|
| config 里 5 条 `en_cta` | 没有它,读取处那个 `L(zh, en)` 分支读不到东西 |
| `Report.tsx` 改成 `L(route.zh_cta, route.en_cta)` | 没有它,文案写了也不会被渲出来 |
| 14 处内部字段加带理由的兄弟键 | 没有它,那道门进 `build` 会红在不该修的地方 |

三件齐了之后 `check:i18n-parity` 才第一次能进构建链 —— 现在是**第十四道门**:
`78 对 zh/en 字段齐全;14 处带理由的豁免`。

## 豁免写在 config 里,不写在门里

```json
"zh_note": "假设 30% 询盘因响应慢或无记录而流失",
"zh_note_i18n": "internal —— 口径假设的说明,只给我们自己看,不渲染给客户"
```

理由是同一条:**白名单在门里,改那一行的人看不到;兄弟键在 config 里,谁改那一行谁就看见**
(与 `check:ghl-transport` 同一条约定)。门把接受的理由**每次构建都打印出来** ——
一个每次都被念一遍的豁免,至少还在视野里。

理由 <8 字符也会红:一个只有名字的豁免半年后没人知道当初为什么在那里。

## 文案不是直译

中文那几条用了「自己会说话」「漏询盘」这类口语,英文照搬会很怪。
英文版按同样的意思重写,保留那个「**先说痛点、再给出路**」的结构 ——
它是这一节的形状,而不是一句话的形状。

## 反向验证:第一条断言先验「抓不抓得到起因」

按[第 0 条](#0-造一个检测器时第一条断言是它能抓到那个促使它诞生的东西)的顺序:

| 验什么 | 结果 |
|---|---|
| **删掉 `offer_routing.convert.en_cta`(起因那个案例)** | **红,点名 offer_routing.convert** ← 第一条 |
| 删掉一处豁免理由 | 红 |
| 豁免理由改成「内部」(2 字) | 红,「理由太短」 |
| 复原 | 绿 |

起因那个案例已经被这一轮修好了,所以验它必须**刻意把它改回去** ——
而那一步最容易被省掉(第 0 条的推论:这也是「先立门再改」这个顺序的理由)。

## 判断标准新增两条

- **第 0 条**(排在第 1 条之前):造检测器时,第一条断言是「它能抓到那个促使它诞生的东西」。
  变异问「它会不会红」,这条问「它对着最初那个案例会不会红」——
  抓不到起因的话,后面所有变异都是在验一个方向错了的东西
- **[判断标准 20 推论二](#20-关于将来会怎样的注释写下的那一刻起就没有任何东西在验证它)**:
  比「关于未来的断言」更近一层的是**「关于当下的警告」**,
  而写警告的人就是要遵守它的那个人 —— 标本就是我那句
  「这一处读错会让渲出来的那份语言不对」,写完之后在同一个函数里实现了它警告的那件事。
  判据:写下「这一处如果 X 就会 Y」的那一刻立刻问「**这一处现在是 X 吗**」

## 未做

- 那 5 条 `en_cta` **只在本地渲染断言层面验过**;真实英文 PDF 第 5 页要等下一次渲染
- 同一次渲染要确认的还有分享卡:上一轮那个 `?lang=` 修复也在等同一份 PDF

Node 367 / Deno 217,**十四道门**全绿。

# 分享卡语言错了第三次 —— 而每次都在不同的一层

## 你那个观测能直接答:路径**按构造**就是两个文件名

`shareCardObjectPath(sessionId, lang, suffix)` = `${session}-${lang}${suffix}`,
所以 zh 与 en 两行的 `share_card_path` 一定不同。按你的判据:**问题在读取侧**。

## 三次,三层

| 第几次 | 哪一层 | 状态 |
|---|---|---|
| ① | `render-pdf` 硬编码 `lang=zh` | 已修 |
| ② | 报告端点的 `currentLang` 只读 `entitlement.lang`、不读 `?lang=` | 已修 |
| ③ | **前端调那个端点时压根没带 `lang`** | ← 这一次 |

也就是说 **②那个修复一次都没被走到**:`reportApi.ts` 拼的 URL 只带 `rt`。

> **一个修好了但调用方不传参数的分支,与没修没有区别。**

而报告页的 CTA 是英文的,因为那句话走 `useT()` —— 它读**页面 URL** 的 `?lang=`;
只有从 **payload** 来的东西(`cardUrl`)才走那个没带参数的请求。
同一页上两个语言来源,一个对一个错 —— 这正是「谁决定语言」散在多层的表现。

## 这一次换取向:让「选哪一张」变成纯计算

按你的建议把两件事拆开:

- **从请求取 lang** —— 仍然是接线(端点 / 前端各一处),本地难测;
- **用 lang 选文件** —— 抽成 `cardsFor(files, lang)`,**纯函数,有断言**。

而后者恰恰是错过三次的那一半。页面自己知道自己的语言,`files` 里两种语言都在,
所以报告页不再依赖端点猜对:`data.cardUrl`(端点的兼容投影)不再被用来渲卡。

**找不到就都是 null,绝不退回另一种语言** —— 递一张别的语言的卡出去,
那张图确实能打开、只是语言不对,而它是客户拿去发朋友圈的东西。宁可整块不渲。

## 两次变异

| 变异 | 结果 |
|---|---|
| `cardsFor` 退回 `files[0]`(重现那个 bug) | 1 条红 |
| 前端不带 `lang`(重现「修好了但没人传」) | `build` 红 |

⚠️ 第二条**只是碰巧被抓到**:`eslint` 报 `currentLang` 未使用。
如果有人把调用和那个 helper 一起删掉,**没有任何东西会红** ——
那类接线(端点入口 / 前端请求拼装)仍然属于本地测不了的一类。
这次能做的是把**能测的那一半真的测了**,而不是宣称整条链都被守住。
...
- 2026-08-07 — **交接整理**:补上四类只活在对话里的东西 —— ①「[判断标准](#判断标准--这个项目反复用到的二十一条从第-0-条起)」七条(每条附它对应的那次返工)②「[七道门](#十四道门--每一道都是撞出来的)」合并成一节,每道门写明**因为撞了什么才加**③ 状态总览按实际进度更正(4/5/6 原本还写着「未开始」,Stage 6 是 15 题不是 24 题)④ 新增「[当前未完成](#当前未完成)」,含已确认未修的 `assessment_entitlements.status` bug
- 2026-07-31 — rev1 初稿
