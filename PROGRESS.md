...
1. **[判断标准](#判断标准--这个项目反复用到的七条)** —— 七条。它们不是格言,是每一条都对应一次真实的返工。
   不读这节,后面很多设计会显得像洁癖。
...
现在标签是图**下方**的一排列表,要移到五个顶点各自旁边。
`buildRadarAxes()`(`src/components/RadarPentagon.tsx`)已经算出每个轴的角度和端点坐标,
标签位置从那里取,**不要另起一套角度计算** —— 那正是 [雷达那三轮](#雷达基准不重合结案视觉-bug不是数据也不是渲染)
栽过的地方(测试自己写了一份副本,于是绿着守一个正在出错的行为)。
新加的断言同样要落到**最终 DOM**(`renderToStaticMarkup`),不是中间层。

顺带守住[设计系统铁律 2](#设计系统辅助元素不能与数据竞争铁律-2):标签是辅助元素,
不能和数据线争视觉权重;维度色只能出现在标签旁那个**带 2px 墨边框的小方块**里
(`check:dim` 会拦其他用法)。
...
- **`pending` 的定时兜底 sweep** —— 目前唯一兜底是 Admin 按钮(靠人发现)。要全自动就加一条
