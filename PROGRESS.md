...
1. **[判断标准](#判断标准--这个项目反复用到的十二条)** —— 十二条。它们不是格言,是每一条都对应一次真实的返工。
   不读这节,后面很多设计会显得像洁癖。
...
当前分支基线:`main` = `71de957`。测试基线:**Node 265 / Deno 124,七道门全绿**。

当前分支基线:`main` = `d90356c`。测试基线:**Node 262 / Deno 124,七道门全绿**。
> ⚠️ 另有两条已 push 未合并的分支:`fix/glyph-scan-coverage`(判断标准 12、字形自检覆盖)
> 与 `feat/is-test-cohort`(is_test)。三条都改了 PROGRESS.md,合并时冲突**一律双方都留**,
> 各自是独立的小节。测试数以合并后实跑为准。
...
- **`check:deno`** 拦下了 `assessment-admin` 里那份**自己的** `RosterRow` 类型 ——
  ...
  这正是「[verify 靠人记得跑](#七道门--每一道都是撞出来的)」那个已知洞会漏掉的东西。
...
截图截的是整个元素,DOM 里有什么就进图。这是**判断标准 12**(「覆盖范围手写的守卫会被代码长到边界外面」,
在 `fix/glyph-scan-coverage` 分支上新增,合并后可在判断标准一节找到)的又一例,只是这次手写的是「哪里**不要**」:**路由长到了名单外面。**
...
Node **262**(+5)/ Deno 124,七道门全绿。**Stage 9 至此收尾。**
...
- 2026-08-07 — **交接整理**:补上四类只活在对话里的东西 —— ①「[判断标准](#判断标准--这个项目反复用到的十二条)」七条(每条附它对应的那次返工)②「[七道门](#七道门--每一道都是撞出来的)」合并成一节,每道门写明**因为撞了什么才加**③ 状态总览按实际进度更正(4/5/6 原本还写着「未开始」,Stage 6 是 15 题不是 24 题)④ 新增「[当前未完成](#当前未完成)」,含已确认未修的 `assessment_entitlements.status` bug
- 2026-07-31 — rev1 初稿
