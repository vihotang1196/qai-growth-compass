...
推论五:**先看被调用方自己的日志,再看调用方拿到的响应。**

> 响应是对方决定给你看的,日志是它做了什么。

标本:标签一个都没打上那次。`assessment-ghl-resync` 一直在打
`resync: skipped 15 CONFIG/AUTH rows` —— **那行日志早就在,而没人看过**,
因为双方都在盯响应体。而响应体当时是
`{"ok":true,"swept":0,"note":"nothing due for retry"}`:
它在新旧两版代码里逐字相同,所以对两种互斥的解释都成立(推论二那一族)。

也就是说:**仪器已经存在,缺的是看它的顺序。**
后来给响应加 `candidates` 是对的,但那件事本身也说明了这条 ——
如果先去看日志,那 15 条 CONFIG 会立刻说明「字段那边的候选被前缀过滤掉了」,
而那正是当时被误当成「查询没挑中」的东西。

排查顺序因此固定成:
1. **被调用方的日志**(它做了什么,包括它跳过了什么);
2. **调用方拿到的响应**(它愿意告诉你什么);
3. 然后才是读代码、比对文件、推理。

而写代码那一侧的对应动作是:**跳过一件事时要留日志,不只体现在返回值里** ——
返回值会被简化、会被中间层重写,日志不会。
...
# 标签写回验过了,而验过之后立刻找出两个洞

## 已验证(真跑,不是结构论证)

| 判据 | 实测 |
|---|---|
| `skippedTestCohort` = 被挑中的 seed 行数 | **15** ✅ |
| 真实那行的标签与分数对得上 | `completed / hot_lead / mismatch / tier_flywheel / weak_value` ✅ |
| 收口在**标签**这条路径上生效 | 15 条 seed 一个请求都没发 ✅ |

`candidates` 那个仪器第一次用就说明了问题:`fields:{raw:15,due:0}` / `tags:{raw:16,due:16}` ——
字段那边 15 条被 CONFIG 前缀挡掉、标签那边 16 条全新。
**而上一轮那个 `{"ok":true,"swept":0}` 对两种互斥的解释都成立。**

⚠️ 仍待确认:**GHL 上肉眼看到那 5 个标签**。`customFields` 那次的教训是 200 不代表写进去了,
所以这一条不算完。

## 洞一(我上一轮埋的):finalize 传 `null`,重答后旧标签永不移除

上一版那行写着 `syncTagsToGhl(..., null, 'finalize')`,注释写着
「首次写回时 applied 必然是空」。**那个「必然」是错的:**

- finalize 用 `upsert(onConflict: session_id)`,只拦「题没答齐」「没交问卷」——
  两者在第一次 finalize 之后**仍然成立**;
- `assessment-quiz` **不拦** completed 之后改答案(它只有 in_progress→survey 那一处状态逻辑)。

所以「重答 → 再 finalize」这条路是通的。而那时:

1. `applied = null` → 差集 `toRemove = []` → **旧档位标签不会被移除**;
2. 紧接着 `ghl_tags_applied` 被覆盖成新的一套 → **那条残留标签的记录被销毁**,
   以后任何 sweep 都不可能知道要移除它。

也就是说,`ghl_tags_applied` 这整个设计要防的那件事(一个人身上挂两个互斥档位标签,
两条 workflow 都触发),会被这一行悄悄绕过。

修法:**读当前值**,不依赖任何关于调用次数的假设。
(与[判断标准 20](#20-关于将来会怎样的注释写下的那一刻起就没有任何东西在验证它) 同族 ——
只是这次的措辞不是「将来会怎样」,而是「这里不可能发生」。同样零个检查者。)

## 洞二(**已上线路径**,两侧同一个):失败时 `*_synced` 不置回 false

`recordFailure` / `recordTagFailure` 原本只写 `last_error` 与 `next_retry_at`。
于是一行已经 `synced = true` 的记录被重算、而这次同步失败时:

- `synced` 还是 `true`;
- 而 sweep 的候选查询要求它是 `false`;
- → **那次失败永远不会被重试,而且没有任何东西会说。**

一次失败因此变成永久失败。语义上 `false` 才是对的:我们刚试着写的东西**没有**落在 GHL 上。

⚠️ **字段那半是一条已上线路径的行为改动** —— 标签那半是照抄它的形状写的,
所以是同一个洞,一起修。不同意的话回滚那一处就行(`ghlWriteback.ts` 的 `patch`)。

## 顺带:一条断言钉在了错的层次上

「测试批次不发请求」那条原本断言 `patch.ghl_tags_synced === undefined` ——
钉的是**实现细节**(patch 里有没有这个键)。改成显式写 `false` 之后它红了,
**而它红的原因不是行为错了,是断言钉错了层次**:要求是语义上的
「绝不能说同步成功了」,`false` 完全满足。改成 `assertEquals(..., false)`,
另加一条专门钉「失败必置 false」的用例(变异去掉那一行 → 2 条红)。

## 判断标准 14 加推论五

> 先看被调用方自己的日志,再看调用方拿到的响应。响应是对方决定给你看的,日志是它做了什么。

标本就是这次:`resync: skipped 15 CONFIG/AUTH rows` **一直在打,而没人看过** ——
双方都在盯响应体。**仪器已经存在,缺的是看它的顺序。**
写代码那一侧的对应动作:**跳过一件事时要留日志,不只体现在返回值里** ——
返回值会被简化、会被中间层重写,日志不会。

Node 334 / Deno **208**(+1),十一道门全绿。
...
# 报告文件按语言分行(B 方案):地基 + `sendMagicLink` 那个 bug

## 迁移路径的判断:**不双写,直接切**

理由不是「数据少」,是 **PDF 与分享卡是派生物**。
双写的适用场景是「数据重建不出来」,而这里重建的成本正好是一次 Lambda 调用 ——
源数据(答案、结果)一个字都不会动。所以那个双写期只带来风险(它本身就是最容易分叉的时候),
不带来保障。

**旧的 `assessment_results.pdf_*` 列这一轮不删**,但不是为了留退路:
删列不可逆,而且没有理由和建表同时做。关键那句是 ——
**一列没人读就不构成第二个真相源;危险的是读它的人,不是列本身。**
所以配套要一道门禁止再读那几列(下一轮做,与 wiring 一起),
等生产上跑通一段时间再单独一条迁移删列。
...
| 你说的 | 落法 |
|---|---|
| 主键 `(session_id, lang)` | 就是主键 —— 「同一个 session 同一种语言只能有一份」由数据库保证,不靠代码记得 |
| sweep 按 `(session_id, lang)` 挑 | 表的粒度就是这个,所以「按 session 挑会重复渲」这件事**不可表示** |
| Roster 显示每种语言各自的状态 | `langStates()` **总是返回全部语言**,缺的那种是 `absent` —— 那正是「zh ✓ / en —」里的「—」 |
| 两个按钮,不做下拉 | `offerableLangs()` 给出该出现「生成 X 版」的语言列表;`downloadableIn()` 给当前语言那份 |

额外两件我加的:

- **分享卡也进这张表**。它渲的是报告页内容(含档位名),所以同样跟语言有关。
  留在 results 上只有一份的话,「中文卡 + 英文报告」会静默出现。
- **语言进对象路径**(`{session}-zh.pdf` / `-en.pdf`)。两种语言写同一个对象的话,
  后渲的覆盖先渲的 —— 而「覆盖」正是 B 要避免的那件事。带上语言之后,
  「切回中文」在存储层就已经是「另一个文件」。

`pdf_attempts` 留在新表里,注释写明**只数失败**:用户主动请求另一种语言不消耗它 ——
而 B 之下他每种语言只会请求一次,所以那个限流问题**自然消失**,不需要新机制。

迁移带断言:存量按 `zh` 迁(**不是猜** —— `render-pdf` 里那两行 goto 就硬编码着 `lang=zh`),
行数不符则整条回滚。

## `sendMagicLink`:语言不再由操作者的界面决定

签名里**刻意删掉了 `lang` 参数** —— 有那个参数就还会有人从别处传一个进来(上一版就是这么坏的)。
现在只从 `target.lang` 读,而 `SEND_COLS` / `SELECT_COLS` 都补上了那一列。
...
`assessment-login-request`(「我丢了链接」)**不再读请求体里的 lang**:
它只拿到一个手机号 / 邮箱,此刻还不能确认调用者就是那个人 ——
据此写库等于让任何人都能翻掉一个陌生人的语言设置。发信语言一律取 `entitlement.lang`。

⚠️ 而我在那里留下的注释说「lang 仍然用于这次响应的措辞」——**那句是错的**:
这个端点的响应体是恒定的(措辞在前端)。**第九道门的 `no-unused-vars` 当场抓了那个变量,
而它抓到的其实是那句注释。**
...
| 变异 | 结果 |
|---|---|
| `ready` 不看有没有 path | 1 条红 |
| `absent` 与 `failed` 合并 | 2 条红 |
| `langStates` 只返回已有的行 | 3 条红 |
| `working` 也给「生成」按钮 | 1 条红 |
| 路径不带语言 | 1 条红 |
| `downloadableIn` 加一条 `?? 任意 ready` 的退路 | **13 条全绿** |

最后那条值得记:它**没被抓到不是因为断言空,而是因为那个变异本身是空操作** ——
`langStates` 总是返回全部语言,所以那条 `??` 退路永远走不到。
换成真的会退回错语言的写法(`find(s => s.availability === 'ready')`)之后,立刻红。

这是[判断标准 1 推论三](#1-一道没见过它变红的门不值钱)的第三种绿的**镜像**:
那条说「断言可能无事可做」,这条说**变异也可能无事可做** ——
而两者在报告里长得一样(都是「改了代码,测试还绿」)。
所以变异之后要多问一句:**这个变异真的改变了行为吗?**

## 未做(下一轮)

- **四处 wiring**:`render-pdf`(两处硬编码 `lang=zh`)、sweep 的候选查询、Roster 那一格、
  报告页的两个按钮
- **`check:pdf-columns`** 那道门(禁止再读 `assessment_results.pdf_*`)
- 迁移未应用(第四条了:标签四列 / lang / warnings+RPC / report_files)

Node **347**(+5)/ Deno 216,十一道门全绿。
...
- 2026-08-12 — **标签写回实测通过(15 条 seed 全被收口拦下、真实那行 5 个标签与分数对得上),
  ...
  注释写「首次写回时必然是空」—— 而 finalize 是 `upsert`、只拦「题没答齐/没交问卷」,
  quiz 也不拦 completed 之后改答案,所以「重答 → 再 finalize」通:旧档位标签不会被移除,
  且 `ghl_tags_applied` 被覆盖后**残留标签的记录被销毁**。改成读当前值
  ②**已上线路径**:`recordFailure` 失败时不把 `ghl_synced` 置回 false,
  于是已 synced 的行重算失败后 sweep 永远挑不到它 —— 一次失败变永久失败。两侧一起修
  ③一条断言钉在了错的层次(`ghl_tags_synced === undefined` 钉的是实现细节,
  要求其实是「绝不能是 true」)
  ④新增判断标准 14 推论五:**先看被调用方的日志,再看调用方拿到的响应** ——
  `resync: skipped 15 CONFIG/AUTH rows` 一直在打而没人看过,仪器早就在,缺的是看它的顺序
